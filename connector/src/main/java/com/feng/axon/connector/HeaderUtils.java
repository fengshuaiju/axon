...
public abstract class HeaderUtils {
    ...
    /**
     * Creates a new {@link RecordHeader} based on {@code key} and
     ...
     */
    public static void addHeader(org.apache.kafka.connect.header.Headers headers, String key, Object value) {
        notNull(headers, "headers may not be null");
        if (value instanceof Instant) {
            headers.addBytes(key, toBytes((Number) ((Instant) value).toEpochMilli()));
        } else if (value instanceof Number) {
            headers.addBytes(key, toBytes((Number) value));
        } else if (value instanceof String) {
            headers.addBytes(key, ((String) value).getBytes(UTF_8));
        } else if (value == null) {
            headers.addBytes(key, null);
        } else {
            headers.addBytes(key, value.toString().getBytes(UTF_8));
        }
    }
//
//    /**
